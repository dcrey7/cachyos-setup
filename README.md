...
- KWin effects: Magic Lamp, Wobbly Windows, Glide, Sheet, Fade Desktop, Cube, tuned blur, and a custom Cover Switch 2 zoom-in activation effect.
- Cover Switch 2 tabbox layout installed under the distinct `coverswitch2` package ID, so it can coexist with the system `coverswitch`; it refreshes the bottom panel reserve from KWin at Alt+Tab runtime, uses thumbnail zoom open/close transitions, and wraps through windows while forcing wrap animations to travel through the stack.
